...
describe('Menu', () => {
  it('renders cards from HistoryStore', () => {
    ...
    HistoryStore.history = [
      makeImageData({ url: 'https://example.com/1.jpg' }),
      makeImageData({ url: 'https://example.com/2.jpg' }),
    ]
    ...
    expect((cards[0].querySelector('img') as HTMLImageElement).src).toContain(
  ...
  it('marks card images loaded and uses fallback title, image url, and status labels', () => {
    ...
    const fallbackCardImage = document.querySelectorAll(
      '.history-panel .card img',
  ...
  it('settings color presets and custom color update the theme store', async () => {
    ...
    expect(screen.getByRole('button', { name: /set primary color to #ff6b6b/i })).toHaveClass(
      'active',
    )
