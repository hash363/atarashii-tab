...
const pickImageUrl = (post: RedditPost, targetWidth: number, targetHeight: number) => {
  ...
  const candidate =
    candidates.find(({ width, height }) => width >= targetWidth && height >= targetHeight) ||
    candidates[candidates.length - 1]

  return candidate ? decode(candidate.url) : post.url
}

export const createImageData = (post: RedditPost, index: number, total: number): ImageData => {
...
export const pushPostToHistory = (post: RedditPost, index: number, total: number) => {
...
export const clearHistory = () => {
  ...
  if (current >= 0 && HistoryStore.history[current]?.favorite) {
    HistoryStore.i =
      HistoryStore.history.slice(0, current + 1).filter(({ favorite }) => favorite).length - 1
  } else {
